...
@section('content')
  <br>
  <div class="row" id="kachaee-payment">
          ...
          <div class="row hideOnPrint">
            <div class="col-sm-12 hideOnPrint">
              <div class="all-form-element-inner">
                @if(!$paymentEdit)
                  <form action="/dashboard/kachaee-payments" method="post">
                    @csrf
                    ...
                    <div class="row">
                      ...
                      <div class="col-lg-1 col-md-1 col-sm-12 col-xs-12" style="margin-top: 10px">
                        ...
                        <select name="kachaee_number" id="kachaee_number" class="form-control">
                          <option value="نقد">نقد</option>
                          @foreach($kachaee_numbers as $ch)
                            <option value="{{$ch->kachaee_number}}">{{$ch->kachaee_number}}</option>
                          @endforeach
                        </select>
                        
                        @error('kachaee_number') <p class="text-danger">
                ...
                @else
                  <form action="/dashboard/kachaee-payments/{{$paymentEdit->id}}" method="post">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="team_id" value="{{$team->id}}">
                    ...
                    <div class="row">
                      ...
                      <div class="col-lg-1 col-md-1 col-sm-12 col-xs-12" style="margin-top: 10px">
                        ...
                        <select name="kachaee_number" id="kachaee_number" class="form-control">
                          <option value="نقد">نقد</option>
                          @foreach($kachaee_numbers as $ch)
                            <option {{ $paymentEdit->kachaee_number ==  $ch->kachaee_number  ? 'selected' : '' }}  value="{{$ch->kachaee_number}}">{{$ch->kachaee_number}}</option>
                          @endforeach
                        </select>
          ...
          <div class="row">
            ...
            <div class="col-lg-3 col-md-3 col-sm-3 col-xs-3 hideOnPrint">
              <div class="btn-group hideOnPrint" id="exportButton" style="float: left; ">
                <div class="btn btn-sm btn-primary" style="float: left" onclick="printPage('kachaee-payment')"><i
                          class="fa fa-print"></i> چاپ
                </div>
              
              </div>
              <a href="/dashboard/kachaee-payments-all/{{$team->id}}" style="float: left"
        ...
        <div class="card-body">
          <div class="row">
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
              ...
              <div class="table-responsive">
                <table class="table table-xs table-hover" id="kachaee_payment">
                  <thead>
                  <tr>
                    <td><b>رسید(دالر)</b></td>
                    <td><b>گرفت(دالر)</b></td>
                  ...
                  @foreach($payments as $pa)
                    <tr class="ur{{$pa->id}}">
                      ...
                      @else
                        <td>0</td>
                      @endif
                      @if($pa->kachaee_number == 'نقد')
                        <td>نقد</td>
                      @else
                        
                        
                        <td>
                          <a href="/dashboard/carpet-repair/search-kachaee-number/{{$pa->kachaee_number}}{{$pa->team_id}}"
                          >&nbsp; {{$pa->kachaee_number}}</a></td>
                      ...
                      @if( $pa->status == 0 || auth()->user()->role == 'SP')
                        <td class="hideOnPrint text-center">
                          <a href="/dashboard/kachaee-payments/{{$pa->id}}/edit"
                             class="btn btn-sm btn-info">ویرایش</a>
                          
                          <button onclick="deletePayment({{$pa->id}}, {{$pa->team_id}})" class="btn btn-danger btn-sm ">
                            <i
...
@section('scripts')
  ...
  <script>
      ...
      $(document).ready(function () {
          ...
          var $buttons = $('#kachaee_payment').find('caption').children().detach();
                      ...
                      $.ajax({
                          ...
                          success: function (res) {
                              ...
                              if (res.status == 'success') {
                                  $('.ur' + id).hide();
                                  $('.alert-success').show();
                                  window.location = '/dashboard/kachaee-payments/' + team_id
                              } else {
                                  $('.alert-danger').show();
                              }
