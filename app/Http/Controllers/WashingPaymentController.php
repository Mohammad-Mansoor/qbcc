...
class WashingPaymentController extends Controller
{
    ...
    public function updateAllocation(Request $request, $id)
    {
        return DB::transaction(function () use ($request, $id) {
            ...
            $this->accountingService->reverseTransactionBySource($allocation->id, 'Washing Allocation Edited', 'App\WashingPaymentAllocation');
            ...
            if ($doc) {
                $this->accountingService->postAutoTransaction('washing_advance_settlement', 'ADVANCE_SETTLEMENT', [
                    ...
                    'description' => "ویرایش تصفیه بل شست‌وشو " . ($doc->reference_number ?? '') . " از پیش‌پرداخت شماره " . $payment->id,
                    'source_id' => $allocation->id,
                    'source_type' => 'App\WashingPaymentAllocation',
                ]);
            }
