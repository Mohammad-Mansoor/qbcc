@extends('dsh.master')
@section('title', 'Weight Carpet Registry - QBIC Forensic ERP')
        ...
        <div class="col-sm-12">
            ...
            <div class="glass-card p-4 mb-4 d-flex justify-content-between align-items-center">
                <div>
                    ...
                    <p class="text-muted mb-0 small"><i class="feather icon-anchor mr-1"></i> مدیریت و ارزش‌گذاری قالین‌های
                        وزنی با متدولوژی Forensic FX</p>
                </div>
                <div class="d-flex">
                    <button class="btn btn-primary rounded-lg shadow px-4 py-2" data-toggle="modal"
                        data-target="#weightCarpetModal">
                        ...
                        <form action="/dashboard/list-weight/search" method="post">
                            ...
                            <div class="input-group">
                                <input type="text" name="search" class="form-control rounded-left"
                                    placeholder="جستجوی نمبر پارچه، نقشه، کاریگر..." required value="{{ $search ?? '' }}">
            ...
            @if(isset($agent_id))
                <div class="row mb-4">
                    ...
                    <div class="col-md-3">
                        <div class="glass-card p-3 text-center border-left border-info"
                    ...
                    <div class="col-md-3">
                        <div class="glass-card p-3 text-center border-left border-primary"
                            style="border-left-width: 4px !important;">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="font-weight-bold">
                                            {{ $carpet->name ?? ($carpet->agent->user->name ?? 'N/A') }}</div>
                                        <div class="small text-muted">{{ $carpet->employee_name }}</div>
                                    </td>
                                    <td>
                                        <div class="small">
                                            {{ $carpet->order_number ?? ($carpet->order->order_number ?? 'N/A') }}</div>
                                        <div class="small text-muted">
                                            {{ $carpet->carpet_type ?? ($carpet->type->carpet_type ?? '') }}</div>
                                    </td>
                                    <td>
                                        <div class="small">نقشه: {{ $carpet->map_number }}</div>
                                        <div class="small text-muted">کوالتی:
                                    ...
                                    <td dir="ltr">
                                        ...
                                        <div class="small text-info">
                                            {{ number_format($carpet->total_price_af, 2) }}
                                    ...
                                    <td class="hideOnPrint">
                                        <div class="btn-group">
                                            <a href="/dashboard/edit-weight/{{ $carpet->carpet_id }}"
                                                class="btn btn-sm btn-light-primary" title="ویرایش">
                                                <i class="feather icon-edit"></i>
                                            </a>
                                            @if($carpet->status == 0)
                                                <button
                                                    onclick="openPassModal({{ $carpet->carpet_id }}, '{{ $carpet->parcha_number }}', '{{ $carpet->name ?? ($carpet->agent->user->name ?? '') }}')"
                                                    class="btn btn-sm btn-light-success" title="پاس کردن">
                                                    <i class="feather icon-check-circle"></i>
                                                </button>
                                            @endif
                                            <a href="/dashboard/show-weight/{{ $carpet->carpet_id }}"
                                                class="btn btn-sm btn-light-info" title="جزییات">
    ...
    <div class="modal fade" id="weightCarpetModal" tabindex="-1" role="dialog">
        <div class="modal-dialog modal-xl" role="document">
            <div class="modal-content border-0 shadow-lg" style="border-radius: 20px;">
                <form
                    action="{{ $editCarpet ? '/dashboard/upd-weight/' . $editCarpet->carpet_id : '/dashboard/post-weight' }}"
                    ...
                    <div class="modal-header {{ $editCarpet ? 'bg-info' : 'bg-primary' }} text-white p-4"
                        ...
                        <button type="button" class="close text-white" data-dismiss="modal"
                            onclick="{{ $editCarpet ? "window.location='/dashboard/list-weight'" : "" }}">&times;</button>
                    </div>
                    <div class="modal-body p-4 bg-light">
                        ...
                        <div class="row">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="order_id" class="form-control select2">
                                            ...
                                            @foreach($orders as $ord)
                                                <option value="{{$ord->co_id}}" {{ ($editCarpet && $editCarpet->order_id == $ord->co_id) ? 'selected' : '' }}>
                                                    {{$ord->order_name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="col-md-4">
                                <div class="glass-card p-3 mb-3">
                                    <h6 class="font-weight-bold text-primary mb-3"><i
                                            class="feather icon-settings mr-1"></i> مشخصات فنی</h6>
                                    <div class="row">
                            ...
                            <div class="col-md-4">
                                <div class="glass-card p-3 mb-3 shadow" style="border-right: 4px solid var(--QBIC-accent);">
                                    <h6 class="font-weight-bold text-primary mb-3"><i
                                            class="feather icon-dollar-sign mr-1"></i> ارزش‌گذاری مالی (Forensic)</h6>
                                        ...
                                        <select name="currency_id" id="modal_currency_id"
                                            ...
                                            @foreach($currencies as $curr)
                                                <option value="{{ $curr->id }}" data-rate="{{ $curr->exchange_rate }}"
                                                    data-code="{{ $curr->code }}" {{ ($editCarpet && $editCarpet->currency_id == $curr->id) ? 'selected' : ($curr->code == 'USD' ? 'selected' : '') }}>{{ $curr->code }} ({{ $curr->symbol }})</option>
                                            @endforeach
                                        </select>
                                    </div>
                                            ...
                                            <div class="col-6">
                                                <select name="warehouse_id" class="form-control small">
                                                    @foreach($warehouses as $w)
                                                        <option value="{{ $w->id }}" {{ (old('warehouse_id', $defaultWarehouseId) == $w->id || ($editCarpet && $editCarpet->warehouse_id == $w->id)) ? 'selected' : '' }}>
                                                            {{ $w->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                    ...
                                    <div class="forensic-snapshot">
                                        <div class="d-flex justify-content-between mb-2">
                                            ...
                                            <span id="label_area"
                                                class="font-weight-bold text-dark">{{ $editCarpet ? $editCarpet->area : '0.00' }}
                                                m²</span>
                                        ...
                                        <div class="d-flex justify-content-between mb-2">
                                            ...
                                            <span id="label_total_native"
                                                class="font-weight-bold text-info">{{ $editCarpet ? number_format($editCarpet->total_price_af, 2) : '0.00' }}</span>
                                        ...
                                        <div class="d-flex justify-content-between pt-2 border-top border-secondary">
                                            ...
                                            <span id="label_total_usd"
                                                class="h6 font-weight-bold text-dark mb-0">${{ $editCarpet ? number_format($editCarpet->total_price, 2) : '0.00' }}</span>
                        ...
                        <div class="row mt-2">
                            <div class="col-md-3 form-group">
                                ...
                                <input type="date" name="date" class="form-control premium-input"
                                    value="{{ $editCarpet ? $editCarpet->date : date('Y-m-d') }}">
                            </div>
                            <div class="col-md-3 form-group">
                    ...
                    <div class="modal-footer bg-white p-4" style="border-radius: 0 0 20px 20px;">
                        <button type="button" class="btn btn-light rounded-lg px-4" data-dismiss="modal"
                            onclick="{{ $editCarpet ? "window.location='/dashboard/list-weight'" : "" }}">انصراف</button>
                        <button type="submit"
