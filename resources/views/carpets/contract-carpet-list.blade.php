@extends('dsh.master')
@section('title', 'Contract Carpet Registry - QBIC Forensic ERP')
        ...
        <div class="col-sm-12">
            ...
            <div class="glass-card p-4 mb-4 d-flex justify-content-between align-items-center">
                <div>
                    ...
                    <p class="text-muted mb-0 small"><i class="feather icon-activity mr-1"></i> مدیریت موجودی در حال بافت و
                        ارزش‌گذاری اسعاری (Forensic Ready)</p>
                </div>
                <div class="d-flex">
                    <button class="btn btn-primary rounded-lg shadow px-4" data-toggle="modal"
                        data-target="#createCarpetModal">
                        ...
                        <form action="/dashboard/contract-carpet/search" method="post">
                            ...
                            <div class="input-group">
                                <input type="text" name="search" class="form-control rounded-left"
                                    placeholder="جستجوی نمبر پارچه، نقشه یا زمینه..." required>
                        ...
                        <tbody>
                            @foreach($carpets as $carpet)
                                <tr class="ur{{ $carpet->carpet_id }}">
                                    ...
                                    <td>
                                        <div class="small">{{ $carpet->carpet_type ?? ($carpet->type->carpet_type ?? '') }}
                                        </div>
                                        <div class="small text-muted">
                                            {{ $carpet->quality ?? ($carpet->quality->quality ?? '') }}</div>
                                    </td>
                                    <td>
                                    ...
                                    <td dir="ltr">
                                        <span
                                            class="text-primary font-weight-bold">${{ number_format($carpet->price, 2) }}</span>
                                    </td>
                                    <td dir="ltr">
                                        <div class="font-weight-bold text-dark">${{ number_format($carpet->total_price, 2) }}
                                        </div>
                                    ...
                                    <td>
                                        @if($carpet->carpet_image)
                                            <img src="/{{$carpet->carpet_image}}" class="rounded shadow-sm"
                                                style="height: 35px; width: 35px; object-fit: cover; cursor: pointer;"
                                                onclick="showFullImage('/{{$carpet->carpet_image}}')">
                                        @else
                                    ...
                                    <td class="hideOnPrint">
                                        <div class="btn-group">
                                            <a href="/dashboard/contract-carpet/{{ $carpet->carpet_id }}/edit"
                                                class="btn btn-sm btn-light-primary" title="ویرایش (Edit)">
                                                <i class="feather icon-edit"></i>
                                            </a>
                                            <button
                                                onclick="openPassModal({{ $carpet->carpet_id }}, '{{ $carpet->parcha_number }}', '{{ $carpet->agent->user->name ?? '' }}')"
                                                class="btn btn-sm btn-light-success"
                                                title="پاس کردن / تکمیل تولید (Pass/Complete)">
                                                <i class="feather icon-check-circle"></i>
                                            </button>
                                            <a href="/dashboard/contract-carpet/{{ $carpet->carpet_id }}"
                                                class="btn btn-sm btn-light-info" title="جزییات بیشتر (Details)">
    ...
    <div class="modal fade" id="createCarpetModal" tabindex="-1" role="dialog">
        <div class="modal-dialog modal-xl" role="document">
            <div class="modal-content border-0 shadow-lg" style="border-radius: 20px;">
                <form
                    action="{{ $editCarpet ? '/dashboard/contract-carpet/' . $editCarpet->carpet_id : '/dashboard/contract-carpet' }}"
                    method="post" enctype="multipart/form-data">
                    @csrf
                    @if($editCarpet) @method('PATCH') @endif

                    <div class="modal-header {{ $editCarpet ? 'bg-info' : 'bg-primary' }} text-white p-4"
                        ...
                        <button type="button" class="close text-white" data-dismiss="modal"
                            onclick="{{ $editCarpet ? "window.location='/dashboard/contract-carpet'" : "" }}">&times;</button>
                    </div>
                    <div class="modal-body p-4 bg-light">
                        <!-- HIDDEN PILLAR DEFAULTS -->
                        <input type="hidden" name="parcha_number"
                            value="{{ $editCarpet ? $editCarpet->parcha_number : $AccountNo }}">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <div class="glass-card p-3 mb-3">
                                    <h6 class="font-weight-bold text-primary mb-3"><i class="feather icon-user mr-1"></i>
                                        جزئیات تولید</h6>
                                    <div class="form-group">
                                        <label class="form-label-premium">نمبر پارچه</label>
                                        <input type="text" class="form-control bg-white"
                                            value="{{ $editCarpet ? $editCarpet->parcha_number : $AccountNo }}" disabled>
                                    </div>
                                    <div class="form-group">
                                        ...
                                        <select name="agent_id" class="form-control select2" required>
                                            ...
                                            @foreach($agents as $ag)
                                                <option value="{{$ag->agent_id}}" {{ ($editCarpet && $editCarpet->agent_id == $ag->agent_id) ? 'selected' : '' }}>
                                                    {{$ag->user->name}} ({{$ag->account_no}})</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="form-group">
                                        <label class="form-label-premium">نام کاریگر</label>
                                        <input type="text" name="employee_name" class="form-control premium-input"
                                            placeholder="اسم کاریگر"
                                            value="{{ $editCarpet ? $editCarpet->employee_name : '' }}">
                                    </div>
                                    <div class="form-group">
                            ...
                            <div class="col-md-4">
                                <div class="glass-card p-3 mb-3">
                                    <h6 class="font-weight-bold text-primary mb-3"><i
                                            class="feather icon-settings mr-1"></i> مشخصات تخنیکی</h6>
                                    <div class="row">
                                        <div class="col-6 form-group">
                                            <label class="form-label-premium">نوعیت</label>
                                            <select name="type_id" class="form-control">
                                                @foreach($types as $type)
                                                    <option value="{{$type->carpet_type_id}}" {{ ($editCarpet && $editCarpet->type_id == $type->carpet_type_id) ? 'selected' : '' }}>
                                                        {{$type->carpet_type}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-6 form-group">
                                            ...
                                            <select name="quality_id" class="form-control select2">
                                                @foreach($qualities as $q)
                                                    <option value="{{$q->id}}" {{ ($editCarpet && $editCarpet->quality_id == $q->id) ? 'selected' : '' }}>{{$q->quality}}
                                                    </option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-12 form-group">
                                            ...
                                            <input type="text" name="map_number" class="form-control premium-input"
                                                value="{{ $editCarpet ? $editCarpet->map_number : '' }}">
                                        </div>
                                        <div class="col-6 form-group">
                                            <label class="form-label-premium">زمینه</label>
                                            <input type="text" name="field" class="form-control premium-input"
                                                value="{{ $editCarpet ? $editCarpet->field : '' }}">
                                        </div>
                                        <div class="col-6 form-group">
                                            <label class="form-label-premium">حاشیه</label>
                                            <input type="text" name="margin" class="form-control premium-input"
                                                value="{{ $editCarpet ? $editCarpet->margin : '' }}">
                                        </div>
                                        <div class="col-6 form-group">
                                            <label class="form-label-premium">طول (m)</label>
                                            <input type="number" step="0.01" name="height" id="calc_height"
                                                class="form-control premium-input"
                                                value="{{ $editCarpet ? $editCarpet->height : '' }}">
                                        </div>
                                        <div class="col-6 form-group">
                                            <label class="form-label-premium">عرض (m)</label>
                                            <input type="number" step="0.01" name="width" id="calc_width"
                                                class="form-control premium-input"
                                                value="{{ $editCarpet ? $editCarpet->width : '' }}">
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-md-4">
                                <div class="glass-card p-3 mb-3 shadow"
                                    style="border-right: 4px solid var(--QBIC-primary);">
                                    <h6 class="font-weight-bold text-primary mb-3"><i class="feather icon-shield mr-1"></i>
                                        ستون‌های مالی و انبار</h6>
                                        ...
                                        <select name="currency_id" id="modal_currency_id"
                                            ...
                                            @foreach($currencies as $curr)
                                                <option value="{{ $curr->id }}" data-rate="{{ $curr->exchange_rate }}"
                                                    data-code="{{ $curr->code }}" {{ ($editCarpet && $editCarpet->currency_id == $curr->id) ? 'selected' : '' }}>{{ $curr->code }}
                                                    ({{ $curr->symbol }})</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        <label class="form-label-premium">قیمت فی متر (<span
                                                id="ppm_currency_label">USD</span>)</label>
                                        <input type="number" step="0.01" name="price_input" id="calc_ppm"
                                            class="form-control premium-input border-primary"
                                            value="{{ $editCarpet ? ($editCarpet->original_price ?? $editCarpet->price) : '' }}">
                                        <input type="hidden" name="price" id="final_ppm_usd"
                                            value="{{ $editCarpet ? $editCarpet->price : '' }}">
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="warehouse_id" class="form-control">
                                            @foreach($warehouses as $w)
                                                <option value="{{ $w->id }}" {{ ($editCarpet && $editCarpet->warehouse_id == $w->id) ? 'selected' : '' }}>{{ $w->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="override_inventory_account_id" class="form-control">
                                            @foreach($inventoryAccounts as $acc)
                                                <option value="{{$acc->id}}" {{ ($editCarpet ? ($editCarpet->override_inventory_account_id == $acc->id) : ($acc->account_code == '1200')) ? 'selected' : '' }}>{{$acc->account_name}}
                                                    ({{$acc->account_code}})</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div class="forensic-snapshot">
                                        <div class="d-flex justify-content-between mb-2">
                                            ...
                                            <span id="label_area"
                                                class="font-weight-bold">{{ $editCarpet ? $editCarpet->area : '0.00' }}
                                                m²</span>
                                            <input type="hidden" name="area" id="input_area"
                                                value="{{ $editCarpet ? $editCarpet->area : '' }}">
                                        </div>
                                        <div class="d-flex justify-content-between mb-2">
                                            <span class="small font-weight-bold">ارزش (Native):</span>
                                            <span id="label_total_native"
                                                class="font-weight-bold text-primary">{{ $editCarpet ? number_format($editCarpet->total_price_af, 2) : '0.00' }}</span>
                                            <input type="hidden" name="total_price_af" id="input_total_native"
                                                value="{{ $editCarpet ? $editCarpet->total_price_af : '' }}">
                                        </div>
                                        <div class="d-flex justify-content-between">
                                            <span class="small font-weight-bold text-dark">ارزش (Forensic USD):</span>
                                            <span id="label_total_usd"
                                                class="font-weight-bold text-dark">${{ $editCarpet ? number_format($editCarpet->total_price, 2) : '0.00' }}</span>
                                            <input type="hidden" name="total_price" id="input_total_usd"
                                                value="{{ $editCarpet ? $editCarpet->total_price : '' }}">
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="row mt-2">
                            <div class="col-md-3 form-group">
                                ...
                                <input type="date" name="date" class="form-control premium-input"
                                    value="{{ $editCarpet ? $editCarpet->date : '' }}">
                            </div>
                            <div class="col-md-3 form-group">
                                <label class="form-label-premium">تاریخ ختم کار</label>
                                <input type="date" name="end_date" class="form-control premium-input"
                                    value="{{ $editCarpet ? $editCarpet->end_date : '' }}">
                            </div>
                            <div class="col-md-6 form-group">
                                ...
                                @if($editCarpet && $editCarpet->carpet_image)
                                    <div class="mt-2 small text-muted">تصویر فعلی: {{ basename($editCarpet->carpet_image) }}
                                    </div>
                                @endif
                            </div>
                        </div>
                    ...
                    <div class="modal-footer bg-white p-4" style="border-radius: 0 0 20px 20px;">
                        <button type="button" class="btn btn-light rounded-lg px-4" data-dismiss="modal"
                            onclick="{{ $editCarpet ? "window.location='/dashboard/contract-carpet'" : "" }}">انصراف</button>
                        <button type="submit"
                            class="btn {{ $editCarpet ? 'btn-info' : 'btn-primary' }} rounded-lg px-5 shadow-lg font-weight-bold">
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content rounded-xl shadow-lg border-0">
                <div class="modal-body text-center p-5">
                    ...
                    <p>آیا از پاس کردن پارچه <strong id="pass_no"></strong> مربوط به <strong id="pass_agent"></strong>
                        اطمینان دارید؟</p>
                    ...
                    <div class="text-right mt-4 p-3 glass-card bg-light">
                        ...
                        <div class="form-group">
                            ...
                            <select id="pass_inventory_account_id" class="form-control">
                                @foreach($inventoryAccounts as $acc)
                                    <option value="{{$acc->id}}" {{ $acc->account_code == '1201' ? 'selected' : '' }}>
                    ...
                    <p class="text-muted small mt-3">این تراکنش باعث انتقال موجودی از انبار تولید به انبار مرکزی و حساب
                        تعیین شده می‌گردد.</p>
