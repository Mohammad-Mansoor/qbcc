...
<style>
    ...
    .table-modern thead th { background: #f8fafc; color: #64748b; font-size: 11px; text-transform: uppercase; border: none; }
</style>
    ...
    <div class="col-md-4">
        ...
        <div class="glass-card p-4 mb-4">
            ...
            <div class="forensic-snapshot">
                ...
                <div class="d-flex justify-content-between mb-2">
                    ...
                    <span class="text-success font-weight-bold">{{ number_format($carpet->total_price_af, 0) }} AFN</span>
                </div>
                <div class="mt-3 pt-3 border-top small text-muted text-center">
                    <i class="feather icon-clock mr-1"></i> Snapshot Rate: 1 USD = {{ number_format($carpet->exchange_rate, 4) }} {{ $carpet->currency_code ?? 'USD' }}
                </div>
            </div>
        </div>
        ...
        <div class="glass-card p-4">
            ...
            <table class="table table-sm table-borderless mb-0">
                <tr><td class="text-muted small">فروشنده (Vendor):</td><td class="text-right font-weight-bold">{{ $carpet->agent->user->name ?? 'N/A' }}</td></tr>
                <tr><td class="text-muted small">نوعیت:</td><td class="text-right font-weight-bold">{{ $carpet->type->carpet_type ?? 'N/A' }}</td></tr>
                <tr><td class="text-muted small">کوالتی:</td><td class="text-right font-weight-bold">{{ $carpet->quality->quality ?? 'N/A' }}</td></tr>
                <tr><td class="text-muted small">مساحت:</td><td class="text-right font-weight-bold">{{ $carpet->area }} m²</td></tr>
                <tr><td class="text-muted small">ابعاد:</td><td class="text-right font-weight-bold" dir="ltr">{{ $carpet->height }} x {{ $carpet->width }}</td></tr>
                <tr><td class="text-muted small">نقشه:</td><td class="text-right font-weight-bold">{{ $carpet->map_number }}</td></tr>
            </table>
        </div>
    </div>
                ...
                <div class="tab-pane fade show active" id="specs" role="tabpanel">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-12 mb-3">
                            ...
                            <div class="bg-light p-3 rounded-lg font-weight-bold text-dark">{{ $carpet->carpet_order->order_number ?? 'N/A' }}</div>
                        </div>
                        <div class="col-md-6 mb-3">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <div class="bg-light p-3 rounded-lg font-weight-bold text-dark">{{ $carpet->warehouse->name ?? 'N/A' }}</div>
                        </div>
                    </div>
                    ...
                    <div class="mt-4 p-4 border rounded-lg bg-light-primary border-primary border-dashed">
                        <div class="row align-items-center">
                            <div class="col-md-8">
                                <h6 class="font-weight-bold text-primary mb-1">ارزش کل محصولات (Inventory Valuation)</h6>
                                <p class="text-muted small mb-0">مجموع ارزش ثبت شده در دفتر کل (General Ledger) بر اساس نرخ برابری Forensic FX</p>
                            </div>
                            <div class="col-md-4 text-right">
                                <h3 class="font-weight-bold text-primary mb-0">${{ number_format($carpet->total_price, 2) }}</h3>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <form action="/dashboard/check-book" method="post">
                                ...
                                <div class="row">
                                    <div class="col-md-3 form-group">
                                        ...
                                        <select name="currency_id" id="check_currency_id" class="form-control border-primary">
                                            @foreach($currencies as $curr)
                                                <option value="{{ $curr->id }}" data-rate="{{ $curr->exchange_rate }}" data-code="{{ $curr->code }}">{{ $curr->code }}</option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="col-md-3 form-group">
                                        ...
                                        <input type="number" step="0.01" name="kachaee_amount_input" id="check_kachaee_input" class="form-control premium-input border-primary" value="0">
                                        <input type="hidden" name="kachaee_amount" id="final_kachaee_native" value="0">
                                    </div>
                                    <div class="col-md-3 form-group">
                                        <label class="form-label-premium">معادل دالر (USD)</label>
                                        <input type="text" name="kachaee_dollar_amount" id="check_kachaee_usd" class="form-control premium-input bg-light" readonly value="0">
                                    </div>
                                    <div class="col-md-3 form-group">
                                    ...
                                    <div class="col-md-3 form-group">
                                        ...
                                        <input type="number" step="0.01" name="height" id="check_height" value="{{$carpet->height}}" class="form-control premium-input">
                                    </div>
                                    <div class="col-md-3 form-group">
                                        <label class="form-label-premium">عرض نهایی</label>
                                        <input type="number" step="0.01" name="width" id="check_width" value="{{$carpet->width}}" class="form-control premium-input">
                                    </div>
                                    <div class="col-md-3 form-group">
                                        <label class="form-label-premium">مساحت نهایی</label>
                                        <input type="number" step="0.01" name="area" id="check_area" value="{{$carpet->area}}" class="form-control premium-input bg-light" readonly>
                                    </div>
                                    <div class="col-md-3 form-group">
                                        <label class="form-label-premium">تاریخ تایید</label>
                                        <input type="date" name="date" value="{{ date('Y-m-d') }}" class="form-control premium-input">
                                    </div>
                                </div>
                                <button type="submit" class="btn btn-primary rounded-lg px-4 shadow">ذخیره و تایید ابعاد</button>
                            </form>
                        </div>
                    @else
                        <div id="checkView">
                            <table class="table table-bordered table-modern">
                                <tr><th class="bg-light w-25">نمبر چک بٌک</th><td>{{ $carpetCheckBook->check_number }}</td></tr>
                                <tr><th class="bg-light">ابعاد نهایی</th><td dir="ltr" class="font-weight-bold text-dark">{{ $carpetCheckBook->height }} x {{ $carpetCheckBook->width }}</td></tr>
                                <tr><th class="bg-light">مساحت نهایی</th><td class="font-weight-bold text-primary">{{ $carpetCheckBook->area }} m²</td></tr>
                                <tr><th class="bg-light">تاریخ تایید</th><td>{{ $carpetCheckBook->date }}</td></tr>
                            </table>
                        </div>
                    @endif
                </div>
