        ...
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
            <div class="card" id="print_header_card">
                ...
                <div class="card-body">
                    ...
                    <form action="/dashboard/get_kachaee_team_balance_report" method="POST" id="search_form">
                        <div class="row">
                            @csrf
                            <div class="form-group col-lg-3 col-md-3 col-sm-12 col-xs-12" style="margin-top: 34px;">
                                <?php $kachaee_team_accounts = \Illuminate\Support\Facades\DB::table('kachaees')->get(); ?>
                                <select name="team_id" id="customer_select_id" class="form-control">
                                    <option value="" selected>انتخاب حساب کچایی گر</option>
                                    <option value="all"> همه</option>
                                    @foreach($kachaee_team_accounts as $t)
                    ...
                    @if($all_accounts)
                       ...
                       <h4>
                            گزارش بیلانس حسابات کچایی گر ها ازتاریخ {{ $from_date  }} الی {{ $to_date }}
                        </h4>
                        <div class="row">
                            <div class="table-responsive container">
                                ...
                                <table class="table table-xs table-hover customer_demands">
                                    ...
                                    @foreach($all_accounts as $cr)
                                            <?php
                                            $afg_temp = \Illuminate\Support\Facades\DB::table('kachaee_payments')->where('team_id', $cr->id)->where('type', 'رسید')->where('amount_af','>', 0)->whereBetween('date', [$from_date, $to_date])->sum('amount_af')
                                                - \Illuminate\Support\Facades\DB::table('kachaee_payments')->where('team_id', $cr->id)->where('type', 'گرفت')->where('amount_af','>', 0)->whereBetween('date', [$from_date, $to_date])->sum('amount_af');

                                            $usd_temp = \Illuminate\Support\Facades\DB::table('kachaee_payments')->where('team_id', $cr->id)->where('type', 'رسید')->where('amount','>', 0)->whereBetween('date', [$from_date, $to_date])->sum('amount')
                                                - \Illuminate\Support\Facades\DB::table('kachaee_payments')->where('team_id', $cr->id)->where('type', 'گرفت')->where('amount','>', 0)->whereBetween('date', [$from_date, $to_date])->sum('amount');
