            ...
            <div class="card" id="print_header_card">
                ...
                <div class="card-body">
                    ...
                    @if($balances)
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-8 col-md-8 col-sm-8">
                                <h5>بیلانس محترم {{ $agent_name->name }} از تاریخ {{ $from_date }} الی
                                    تاریخ {{ $to_date }}</h5>
                    ...
                    @if($agent)
                        ...
                        <h4>
                            گزارش بیلانس نماینده ها ازتاریخ {{ $from_date  }} الی {{ $to_date }}
                        </h4>
                        <div class="row">
                            <div class="table-responsive container">
                                ...
                                <table class="table table-xs table-hover customer_demands">
                                    ...
                                    <tr>
                                        ...
                                        <th rowspan="2" style="vertical-align: inherit;">نام نماینده</th>
                                    ...
                                    @foreach($agent as $cr)
                                            <?php
                                            $afg_temp = \Illuminate\Support\Facades\DB::table('agent_payments')->where('agent_id', $cr->agent_id)->where('type', 'رسید')->where('amount_af','>', 0)->whereBetween('date', [$from_date, $to_date])->sum('amount_af')
                                                - \Illuminate\Support\Facades\DB::table('agent_payments')->where('agent_id', $cr->agent_id)->where('type', 'گرفت')->where('amount_af','>', 0)->whereBetween('date', [$from_date, $to_date])->sum('amount_af');

                                            $usd_temp = \Illuminate\Support\Facades\DB::table('agent_payments')->where('agent_id', $cr->agent_id)->where('type', 'رسید')->where('amount','>', 0)->whereBetween('date', [$from_date, $to_date])->sum('amount')
                                                - \Illuminate\Support\Facades\DB::table('agent_payments')->where('agent_id', $cr->agent_id)->where('type', 'گرفت')->where('amount','>', 0)->whereBetween('date', [$from_date, $to_date])->sum('amount');

                                            ?>
                                        <tr>
                                            <td>{{$c}}</td>
                                            <?php $user = \Illuminate\Support\Facades\DB::table('users')->where('id',$cr->user_id)->first(); ?>
                                            <td>{{$user->name}}</td>
