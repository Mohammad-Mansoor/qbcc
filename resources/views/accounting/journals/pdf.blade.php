...
<head>
    ...
    <style>
        ...
        table.ledger-table tfoot td { background-color: #e2e8f0 !important; font-weight: bold; border-top: 2px solid #1e3a8a; font-size: 11pt; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
            ...
            <td style="border: none; padding: 0;">
                ...
                <div class="content-wrapper">
                    ...
                    <table class="meta-table">
                        <tr>
                            ...
                            <td style="width: 33%; text-align: center; border-left: 1px solid #e2e8f0;">
                                ...
                                <p class="meta-val-primary" style="color: #0369a1;">{{ $transaction->reference ?? '-' }}</p>
                            </td>
                    ...
                    <div class="description-box">
                        <span style="font-weight: bold; color: #1e3a8a; display: block; margin-bottom: 5px; font-size: 10pt;">شرح سند (Narrative):</span>
                    ...
                    <table class="ledger-table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td class="text-center font-bold" style="color: #475569;">{{ $entry->account->account_code }}</td>
                                <td class="text-right font-bold">{{ $entry->account->account_name }}</td>
                                <td class="text-left font-bold" style="direction: ltr;">
                                    @if($entry->debit > 0)
                                        {{ number_format($entry->debit, 2) }} <small style="color: #64748b;">{{ $entry->currency_code }}</small>
                                        @if($entry->currency_code !== 'USD')
                                            <br><small style="color: #94a3b8; font-size: 8.5pt;">(${{ number_format($entry->base_debit, 2) }})</small>
                                        @endif
                                    @else
                                        -
                                    @endif
                                ...
                                <td class="text-left font-bold" style="direction: ltr;">
                                    @if($entry->credit > 0)
                                        {{ number_format($entry->credit, 2) }} <small style="color: #64748b;">{{ $entry->currency_code }}</small>
                                        @if($entry->currency_code !== 'USD')
                                            <br><small style="color: #94a3b8; font-size: 8.5pt;">(${{ number_format($entry->base_credit, 2) }})</small>
                                        @endif
                                    @else
                                        -
                                    @endif
                        ...
                        <tfoot>
                            <tr>
                                ...
                                <td class="text-left font-bold" style="direction: ltr; color: #1e3a8a;">${{ number_format($transaction->entries->sum('base_debit'), 2) }}</td>
                                <td class="text-left font-bold" style="direction: ltr; color: #1e3a8a;">${{ number_format($transaction->entries->sum('base_credit'), 2) }}</td>
                            </tr>
                        </tfoot>
                    </table>
