...
<body>
    ...
    <div class="wrapper">
                    ...
                    <tr>
                        ...
                        <td class="text-left font-weight-bold">
                            @if($entry->debit > 0)
                                {{ number_format($entry->debit, 2) }} <small
                                    style="color: #666;">{{ $entry->currency_code }}</small>
                                @if($entry->currency_code !== 'USD')
                                    <br><small
                                        style="color: #888; font-size: 11px;">(${{ number_format($entry->base_debit, 2) }})</small>
                                @endif
                            @else
                                -
                            @endif
                        ...
                        <td class="text-left font-weight-bold">
                            @if($entry->credit > 0)
                                ...
                                @if($entry->currency_code !== 'USD')
                                    <br><small
                                        style="color: #888; font-size: 11px;">(${{ number_format($entry->base_credit, 2) }})</small>
                                @endif
                            @else
                                -
                            @endif
        ...
        <div style="margin-top: 50px; font-size: 11px; text-align: left; color: #888;">
            Printed on: {{ date('Y-m-d H:i:s') }} | QBIC Finance System
        </div>
    </div>
