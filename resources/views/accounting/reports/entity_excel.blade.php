        ...
        <tr>
            <td colspan="6" class="report-title" valign="middle">
                @if(request()->get('type') === 'summary')
                    صورت حساب خلاصه {{ trim(explode('(', $config['title'])[0]) }} (Summary {{ isset(explode('(', $config['title'])[1]) ? trim(str_replace(')', '', explode('(', $config['title'])[1])) . ' ' : '' }}Statement)
                @else
                    صورت حساب تفصیلی {{ trim(explode('(', $config['title'])[0]) }} (Detailed {{ isset(explode('(', $config['title'])[1]) ? trim(str_replace(')', '', explode('(', $config['title'])[1])) . ' ' : '' }}Statement)
                @endif
            </td>
        </tr>
