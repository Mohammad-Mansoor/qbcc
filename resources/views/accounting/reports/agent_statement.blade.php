            ...
            <div class="card border-0 shadow-sm" style="border-radius: 15px;">
                <div class="card-body p-4">
                    <div class="row align-items-center mb-4">
                        <div class="col-md-6">
                            <h3 class="font-weight-bold mb-1">صورت حساب نماینده (Agent Statement)</h3>
                            <p class="text-muted mb-0">گزارش تفصیلی دیبت، کریدت و بیلانس نهایی نماینده</p>
                        </div>
                    </div>
                        ...
                        <div class="row align-items-end">
                            <div class="col-md-4">
                                ...
                                <select name="agent_id" class="form-control select2 shadow-sm" required>
                                    ...
                                    @foreach($agents as $a)
                                        <option value="{{ $a->agent_id }}" {{ isset($agent) && $agent->agent_id == $a->agent_id ? 'selected' : '' }}>
                                            {{ $a->account_no }} - {{ $a->display_name }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
    ...
    <div class="row mb-4 no-print">
        <div class="col-md-4">
            <div class="card border-0 shadow-sm p-4 text-center" style="border-radius: 15px; background: #fff; border-bottom: 4px solid #3f51b5 !important;">
                <span class="text-muted small font-weight-bold text-uppercase">مجموع دیبت (Total Debits)</span>
        ...
        <div class="col-md-4">
            <div class="card border-0 shadow-sm p-4 text-center" style="border-radius: 15px; background: #fff; border-bottom: 4px solid #2ed8b6 !important;">
                <span class="text-muted small font-weight-bold text-uppercase">مجموع کریدت (Total Credits)</span>
                <h3 class="font-weight-bold text-success mt-2 mb-0">{{ number_format($entries->sum('credit'), 2) }}</h3>
            </div>
        </div>
        @php 
            $runningBalance = $openingBalance;
            foreach($entries as $entry) { $runningBalance += ($entry->credit - $entry->debit); }
        @endphp
        <div class="col-md-4">
        ...
        <div class="col-md-12">
            <div class="card border-0 shadow-lg printable-document" style="border-radius: 20px; overflow: hidden;">
                ...
                <div class="d-none d-print-block p-5 bg-light">
                    ...
                    <div class="row">
                        <div class="col-6">
                            <span class="small text-muted font-weight-bold d-block text-uppercase">نماینده (Agent):</span>
                            <h4 class="font-weight-bold text-dark mb-0">{{ $agent->display_name }}</h4>
                            <p class="text-muted">کد حساب: {{ $agent->account_no }}</p>
                        </div>
                        <div class="col-6 text-right">
    ...
    <div class="row justify-content-center mt-5">
        <div class="col-md-6 text-center">
            <div class="p-5 bg-white shadow-sm rounded-lg" style="border-radius: 20px;">
                ...
                <h5 class="text-muted">لطفاً ابتدا یک نماینده را انتخاب کنید.</h5>
                <p class="text-muted small">برای مشاهده صورت حساب نماینده، از فیلترهای بالا استفاده نمایید.</p>
            </div>
        </div>
    </div>
    @endif
            ...
            buttons: [
                {
                    ...
                    title: 'Agent Statement - {{ isset($agent) ? $agent->display_name : "" }}'
                },
                {
                    extend: 'pdfHtml5',
