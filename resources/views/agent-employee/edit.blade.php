        ...
        <div class="col-lg-12 col-md-12 col-xs-12 col-sm-12">
            <div class="card">
                <div class="card-header">
                    <h4>ویرایش جزئیات کارگر</h4>
                </div>
                <div class="card-body">
                    <div class="all-form-element-inner">
                        <form action="/dashboard/agent-employees/{{$employee->id}}" method="post">
                            @csrf
                            @method('PUT')
                                    ...
                                    <div class="col-lg-9 col-md-9 col-sm-9 col-xs-12">
                                        <select name="agent_id" class="form-control" id="" required>
                                            @foreach($agents as $agent)
                                                <option {{($agent->agent_id == $employee->agent->agent_id ? 'selected' : '')}} value="{{$agent->agent_id}}">{{$agent->user->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div class="row">
                                        <div class="col-lg-10">
                                            <div class="cancel-wp pull-right">
                                                ...
                                                <a class="btn btn-default btn-sm" href="/dashboard/agent-employees">انصراف</a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
