...
@section('content')
    <div class="container-fluid py-4" style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">
        ...
        <div class="row mb-4">
            <div class="col-lg-4 col-md-6 mb-3">
                <div class="card border-0 shadow-sm"
                    style="background: rgba(255, 255, 255, 0.7); backdrop-filter: blur(10px); border-radius: 12px;">
                    <div class="card-body p-4">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                <h6 class="text-uppercase text-muted mb-2 font-weight-bold" style="font-size: 0.85rem;">کل
                                    طلبات از مشتریان (GL)</h6>
            ...
            <div class="col-lg-4 col-md-6 mb-3">
                <div class="card border-0 shadow-sm"
                    ...
                    <div class="card-body p-4">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                <h6 class="text-uppercase text-muted mb-2 font-weight-bold" style="font-size: 0.85rem;">
                                    مجموع پرداخت‌ها (USD)</h6>
        ...
        <div class="card border-0 shadow-sm mb-4 hideOnPrint"
            ...
            <div class="card-body">
                <form method="post"
                    action="{{ !$customerEdit ? '/dashboard/customers' : '/dashboard/customers/' . $customerEdit->id }}">
                    @if($customerEdit) @method('PATCH') @endif
                    @csrf
                    <div class="row align-items-end">
                        <div class="col-md-2 col-sm-6 mb-3">
                            <div class="form-group mb-0">
                                <label class="font-weight-bold text-muted mb-2" style="font-size: 0.85rem;">کود
                                    مشتری</label>
                                <input type="text"
                                    value="{{ $customerEdit ? $customerEdit->customer_code : old('customer_code') }}"
                                    class="form-control form-control-sm border-light-gray shadow-none" required
                                    name="customer_code" style="border-radius: 8px;">
                            </div>
                        </div>
                        <div class="col-md-2 col-sm-6 mb-3">
                            <div class="form-group mb-0">
                                <label class="font-weight-bold text-muted mb-2" style="font-size: 0.85rem;">نام
                                    مشتری</label>
                                <input type="text" class="form-control form-control-sm border-light-gray shadow-none"
                                    value="{{ $customerEdit ? $customerEdit->name : old('name') }}" required name="name"
                        ...
                        <div class="col-md-2 col-sm-6 mb-3">
                            <div class="form-group mb-0">
                                <label class="font-weight-bold text-muted mb-2" style="font-size: 0.85rem;">نام
                                    کمپنی</label>
                                <input type="text"
                                    value="{{ $customerEdit ? $customerEdit->company_name : old('company_name') }}"
                                    class="form-control form-control-sm border-light-gray shadow-none" required
                                    name="company_name" style="border-radius: 8px;">
                            </div>
                        </div>
                        <div class="col-md-2 col-sm-6 mb-3">
                            <div class="form-group mb-0">
                                <label class="font-weight-bold text-muted mb-2" style="font-size: 0.85rem;">آدرس کمپنی
                                    (موقعیت)</label>
                                <input type="text"
                                    value="{{ $customerEdit ? $customerEdit->company_address : old('company_address') }}"
                                    class="form-control form-control-sm border-light-gray shadow-none" required
                                    name="company_address" style="border-radius: 8px;">
                            </div>
                        </div>
                        <div class="col-md-2 col-sm-6 mb-3">
                            <div class="form-group mb-0">
                                <label class="font-weight-bold text-muted mb-2" style="font-size: 0.85rem;">نمبر
                                    تماس</label>
                                <input type="text" value="{{ $customerEdit ? $customerEdit->phone : old('phone') }}"
                                    class="form-control form-control-sm border-light-gray shadow-none" required name="phone"
                                    style="border-radius: 8px;">
                            </div>
                        </div>
                        <div class="col-md-2 col-sm-6 mb-3">
                            <div class="form-group mb-0">
                                ...
                                <select name="type" class="form-control form-control-sm border-light-gray shadow-none"
                                    style="border-radius: 8px;">
                        ...
                        <div class="col-12 text-left mt-2">
                            <button
                                ...
                                type="submit" style="border-radius: 8px;">
                                <i class="fa fa-save mr-1"></i> {{ !$customerEdit ? 'ثبت مشتری' : 'ذخیره تغییرات' }}
                            </button>
                            @if($customerEdit)
                                <a href="/dashboard/customers" class="btn btn-sm btn-light ml-2 border px-3"
                                    style="border-radius: 8px;">انصراف</a>
                            @endif
                        </div>
                    </div>
                </form>
                    ...
                    <div class="col-md-6 col-sm-12 text-left mt-2 mt-md-0 hideOnPrint">
                        <form action="/dashboard/customers/search" method="post" class="d-inline-block w-75">
                            ...
                            <div class="input-group input-group-sm">
                                <input type="text" name="search" placeholder="جستجو بر اساس نام، کود، کمپنی..."
                                    class="form-control border-light-gray shadow-none" value="{{ $search ?? '' }}"
                            ...
                            @forelse($customers as $cust)
                                <tr>
                                    ...
                                    <td class="text-left font-weight-bold" dir="ltr">
                                        <span
                                            class="{{ ($cust->ledger_balance ?? 0) > 0 ? 'text-success' : (($cust->ledger_balance ?? 0) < 0 ? 'text-danger' : 'text-muted') }}">
                                            ${{ number_format($cust->ledger_balance ?? 0, 2) }}
                                        </span>
                                    </td>
                                    <td class="text-left font-weight-bold" dir="ltr">
                                        <span
                                            class="{{ ($cust->total_usd ?? 0) > 0 ? 'text-success' : (($cust->total_usd ?? 0) < 0 ? 'text-danger' : 'text-muted') }}">
                                            ${{ number_format($cust->total_usd ?? 0, 2) }}
                                        </span>
                                    </td>
                                    <td class="text-left font-weight-bold" dir="ltr">
                                        <span
                                            class="{{ ($cust->total_af ?? 0) > 0 ? 'text-success' : (($cust->total_af ?? 0) < 0 ? 'text-danger' : 'text-muted') }}">
                                    ...
                                    <td>
                                        @if($cust->last_activity)
                                            ...
                                            <small
                                                class="{{ ($cust->days_since_active ?? 0) > 60 ? 'text-danger font-weight-bold' : 'text-muted' }}">
