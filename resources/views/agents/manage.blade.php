@extends('dsh.master')
@section('title', 'Agent Profile - QBIC Forensic ERP')

@section('content')
<style>
    /* QBIC PREMIUM DASHBOARD SYSTEM */
    :root {
        --QBIC-primary: #1e3a8a;
...
<div class="container-fluid">
    ...
    <div class="profile-header">
        <div class="agent-hero">
            <img src="{{ $agent->image ? '/' . $agent->image : 'https://ui-avatars.com/api/?name=' . $agent->user->name . '&background=1e3a8a&color=fff' }}"
                class="hero-avatar">
            <div class="hero-info">
                ...
                <p class="text-muted mb-0 small"><i class="feather icon-map-pin mr-1"></i>
                    {{ $agent->province->province }} - {{ $agent->agent_address }}</p>
            </div>
        </div>
        <div class="hero-actions">
            ...
            @can('deactivate_agent')
            <a href="/dashboard/agent-status-change/{{$agent->agent_id}}"
                class="btn-action-premium {{ $agent->account_status == 1 ? 'btn-light-p text-danger' : 'btn-success text-white' }}">
                <i class="feather icon-power"></i>
                {{ $agent->account_status == 1 ? 'غیر فعال سازی' : 'فعال سازی حساب' }}
            </a>
            @endcan
        </div>
    </div>
    ...
    <div class="row mb-4">
        <div class="col-md-3">
            <div class="stat-card">
                ...
                <span
                    class="stat-value {{ $bal_usd >= 0 ? 'text-success' : 'text-danger' }}">{{ number_format($bal_usd, 2) }}
                    $</span>
            </div>
        </div>
        <div class="col-md-3">
            <div class="stat-card">
                ...
                <span
                    class="stat-value {{ $bal_af >= 0 ? 'text-success' : 'text-danger' }}">{{ number_format($bal_af, 0) }}
                    ؋</span>
            </div>
        </div>
        <div class="col-md-3">
            <div class="stat-card">
                ...
                <span
                    class="stat-value text-primary">{{ $agent->contract_type == 'contractional' ? 'قراردادی' : 'وزنی' }}</span>
            </div>
        </div>
        <div class="col-md-3">
            <div class="stat-card">
    ...
    <div class="row">
        ...
        <div class="col-lg-7">
            <div class="glass-section">
                ...
                <div class="section-body">
                    ...
                    <div class="info-item mb-4">
                        ...
                        <p class="text-dark font-weight-bold">{{ $agent->description ?? 'هیچ توضیحی ثبت نشده است.' }}
                        </p>
                    </div>
                    ...
                    @if($agent->contract_scan_file)
                        <div class="doc-preview">
                            <i class="feather icon-image text-primary"
                                style="font-size: 40px; display: block; margin-bottom: 15px;"></i>
                            <h5 class="font-weight-bold">فایل اسکن شده موجود است</h5>
                            <p class="small text-muted mb-4">آخرین آپدیت در سیستم QBIC ثبت شده است.</p>
                            <div class="d-flex justify-content-center gap-2">
                                <a href="/{{ $agent->contract_scan_file }}" target="_blank"
                                    class="btn btn-primary btn-sm rounded-lg px-4"><i class="feather icon-eye mr-1"></i>
                                    مشاهده فایل</a>
                                <a href="#" class="btn btn-outline-secondary btn-sm rounded-lg px-4"><i
                                        class="feather icon-refresh-cw mr-1"></i> آپدیت جدید</a>
                            </div>
                        </div>
                    @else
                        <div class="doc-preview" style="border-color: #fee2e2; background: #fffafb;">
                            <i class="feather icon-alert-circle text-danger"
                                style="font-size: 40px; display: block; margin-bottom: 15px;"></i>
                            ...
                            <button class="btn btn-danger btn-sm rounded-lg px-4"><i
                                    class="feather icon-upload-cloud mr-1"></i> آپلود فایل</button>
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-6">
                    <a href="/dashboard/agent-payments/{{$agent->agent_id}}"
                        class="stat-card d-block text-decoration-none mb-4">
                ...
                <div class="col-md-6">
                    <a href="/dashboard/agent-carpet/{{$agent->agent_id}}"
                        class="stat-card d-block text-decoration-none mb-4">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
        ...
        <div class="col-lg-5">
            ...
            <div class="glass-section">
                <div class="section-header d-flex justify-content-between align-items-center">
                    ...
                    <button onclick="OpenFormBox()" class="btn btn-sm btn-light-primary rounded-lg add-new-phone"><i
                            class="feather icon-plus"></i> جدید</button>
                </div>
                <div class="section-body">
                    @foreach($agent->phone as $index => $ph)
                        <div class="phone-pill">
                            ...
                            @if($index > 0)
                                <button class="btn btn-sm text-danger shadow-none p-0"
                                    onclick="window.location.href = '/dashboard/agent/phone/{{ $ph->phone_id }}'">
                        ...
                        <form action="/dashboard/agent/phone/{{ $agent->agent_id }}" method="post">
                            ...
                            <div class="input-group">
                                <input type="text" id="phone_no" name="phone_no"
                                    class="form-control form-control-modern" dir="ltr" placeholder="(99) 99-999-999">
                                <div class="input-group-append">
                                    <button type="submit" class="btn btn-primary px-3"><i
                                            class="feather icon-save"></i></button>
                                    <button type="button" class="btn btn-light px-3" onclick="CloseFormBox()"><i
