...
@section('content')
  ...
  <style>
    ...
    .nav-pills .nav-link.active {
      background-color: var(--QBIC-primary, #3b82f6) !important;
  ...
  <div class="row mb-4">
    ...
    <div class="col-xl-3 col-md-6 mb-4">
      <div class="card stat-card stat-card-blue p-4 h-100">
        <div class="d-flex justify-content-between align-items-start">
          <div>
            <div class="stat-card-val">{{$pending_pcs}} <span style="font-size: 1rem; font-weight: normal;">عدد</span>
            </div>
    ...
    <div class="col-xl-3 col-md-6 mb-4">
      <div class="card stat-card stat-card-indigo p-4 h-100">
        <div class="d-flex justify-content-between align-items-start">
          <div>
            <div class="stat-card-val">{{number_format($pending_area, 2)}} <span
    ...
    <div class="col-xl-3 col-md-6 mb-4">
      <div class="card stat-card stat-card-green p-4 h-100">
        <div class="d-flex justify-content-between align-items-start">
          <div>
            <div class="stat-card-val">{{$completed_pcs}} <span style="font-size: 1rem; font-weight: normal;">عدد</span>
            </div>
        ...
        <div class="card-body p-4">
          <ul class="nav nav-pills mb-4 pb-3 border-bottom" id="pills-tab" role="tablist">
            <li class="nav-item mr-2">
              <a class="nav-link {{$wash_check == 1 ? 'active' : ''}}" id="pills-non-washed-tab" data-toggle="pill"
                href="#non-washed" role="tab">
                <i class="fa fa-list mr-1"></i> لیست شست نمبر ها (Wash Bills)
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link {{$wash_check == 2 ? 'active' : ''}}" id="pills-washed-tab" data-toggle="pill"
                href="#washed" role="tab">
            ...
            <div class="tab-pane fade {{$wash_check == 1 ? 'active show' : ''}}" id="non-washed" role="tabpanel"
              ...
              <div class="row align-items-center mb-4">
                ...
                <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12 text-left">
                  <button class="btn btn-light border shadow-sm" onclick="printPage('noneRepairPrint')"><i
              ...
              <div class="row align-items-center mb-4 search-modern">
                ...
                <div class="col-md-4 mb-2 mb-md-0">
                  <form action="/dashboard/carpet-wash/search" method="POST" class="w-100">
                    ...
                    <div class="input-group">
                      <input type="text" name="search" placeholder="جستجو شماره قالین، شست نمبر، کاریگر..."
                ...
                <div class="col-md-4 mb-2 mb-md-0">
                  <form action="/dashboard/carpet-wash/search-carpet-type" method="POST" class="w-100">
                    ...
                    <div class="d-flex align-items-center">
                      ...
                      <select name="carpet_type_id" id="carpet_type_id" class="form-control"
                ...
                <div class="col-md-4 text-left">
                  @if(request('search') || request('carpet_type_id'))
                    <a href="/dashboard/carpet-wash" class="btn btn-clear py-2 px-3"><i class="fa fa-times ml-1"></i> پاک
                      کردن فیلتر</a>
                  @endif
                  <button class="btn btn-light border shadow-sm" onclick="printPage('repairPrint')"><i
                      class="fa fa-print mr-1"></i> Print</button>
                </div>
              </div>
                      ...
                      @if($washed->carpet)
                        <tr class="ur{{ $washed->id }}">
                          ...
                          <td><small class="text-muted"
                              title="{{ $washed->description }}">{{ Str::limit($washed->description, 20) }}</small></td>

                          <!-- Status Badge -->
                          <td>
                            @if($washed->carpet->status == 13)
                              <span class="badge badge-success px-2 py-1"
                                style="background: rgba(16, 185, 129, 0.1); color: #10b981;">شسته شده</span>
                            @elseif($washed->carpet->status == 0)
                              <span class="badge badge-success px-2 py-1"
                                style="background: rgba(40, 167, 69, 0.1); color: #28a745;">در نزد نماینده</span>
                            @elseif($washed->carpet->status == 1)
                              <span class="badge badge-warning px-2 py-1"
                                style="background: rgba(245, 158, 11, 0.1); color: #f59e0b;">در گدام مرکزی</span>
                            @elseif($washed->carpet->status == 2)
                              <span class="badge badge-danger px-2 py-1"
                                style="background: rgba(220, 53, 69, 0.1); color: #dc3545;">در کچایی نشده ها</span>
                            @elseif($washed->carpet->status == 12)
                              <span class="badge badge-primary px-2 py-1"
                                style="background: rgba(59, 130, 246, 0.1); color: #3b82f6;">در کچایی شده ها</span>
                            @elseif($washed->carpet->status == 3)
                              <span class="badge badge-warning px-2 py-1"
                                style="background: rgba(245, 158, 11, 0.1); color: #f59e0b;">در شست نشده ها</span>
                            @elseif($washed->carpet->status == 4)
                              <span class="badge badge-info px-2 py-1"
                                style="background: rgba(23, 162, 184, 0.1); color: #17a2b8;">در بخش تیاری</span>
                            @elseif($washed->carpet->status == 5)
                              <span class="badge badge-dark px-2 py-1"
                                style="background: rgba(52, 58, 64, 0.1); color: #343a40;">در گدام فروشات</span>
                            @elseif($washed->carpet->status == 6)
                              <span class="badge badge-success px-2 py-1"
                                style="background: rgba(40, 167, 69, 0.1); color: #28a745;">فروخته شده</span>
                            @endif
                          </td>
                          ...
                          <td class="hideOnPrint">
                            <div class="d-flex justify-content-center align-items-center">
                              @can('edit_carpet_wash')
                                <a href="/dashboard/carpet-wash/{{$washed->id}}/edit" class="mx-1" data-toggle="tooltip"
                                  title="ویرایش">
                              ...
                              @if ($washed->carpet->status == 13)
                                @can('send_carpet_to_finishing')
                                  <button type="button" class="btn-confirm-action mx-1"
                                    style="background: none; border: none; padding: 0; cursor: pointer;" data-toggle="tooltip"
                                    title="ارسال به بخش تیاری"
                                    data-action-url="/dashboard/carpet-wash/sent-to-finish/{{$washed->carpet->carpet_id}}"
                                    data-carpet-no="{{$washed->carpet->carpet_no}}" data-action-label="ارسال به بخش تیاری"
                                    data-action-sublabel="Send to Finishing Section" data-modal-icon="fa fa-paper-plane"
                                    data-btn-class="success" data-needs-warehouse="1" data-needs-tayaari="1">
                                ...
                                @can('send_carpet_to_kachaee')
                                  <button type="button" class="btn-confirm-action mx-1"
                                    ...
                                    data-carpet-no="{{$washed->carpet->carpet_no}}" data-action-label="ارسال به کچایی"
                                    data-action-sublabel="Send to Kachaee Section (Repair)" data-modal-icon="fa fa-wrench"
                                    data-btn-class="primary" data-needs-warehouse="1" data-needs-kachaee="1">
                              ...
                              @if($washed->carpet->status == 13 || $washed->carpet->status == 3)
                                  ...
                                  @if($kachaee)
                                    ...
                                    <button type="button" class="btn-confirm-action mx-1"
                                      ...
                                      data-carpet-no="{{$washed->carpet->carpet_no}}" data-action-label="بازگشت به کچایی"
                                  ...
                                  @else
                                    ...
                                    <button type="button" class="btn-confirm-action mx-1"
                                      ...
                                      data-carpet-no="{{$washed->carpet->carpet_no}}" data-action-label="بازگشت به مرکزی"
