@extends('dsh.master')
@section('title', 'Miscellaneous Accounts - QBIC Forensic ERP')

@section('content')
    <style>
        /* QBIC PREMIUM GLASSMORPHISM SYSTEM */
        :root {
            --QBIC-primary: #1e3a8a;
        ...
        <div class="col-sm-12">
            ...
            <div class="page-header-modern d-flex justify-content-between align-items-center">
                <div>
                    ...
                    <p class="text-muted mb-0 small"><i class="feather icon-briefcase text-primary mr-1"></i> مدیریت اشخاص و
            ...
            <div class="stats-container">
                <div class="stat-card">
                    ...
                    <div class="stat-info">
                        <span class="label">بدهی شرکت (به اشخاص)</span>
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-info">
                        <span class="label">طلب شرکت (از اشخاص)</span>
                            ...
                            <form action="/dashboard/different-account/search" method="post" class="d-flex">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" name="search" value="{{ $search ?? '' }}"
                                        placeholder="جستجو بر اساس نام، شماره یا آدرس..." class="form-control border-left-0"
                                        style="width: 300px; border-radius: 0 8px 8px 0;">
                                </div>
                            </form>
                        </div>
                        <div class="hideOnPrint">
                            <button class="btn btn-outline-secondary btn-sm rounded-lg px-3"
                                onclick="printPage('accounts')">
                            ...
                            <tbody>
                                @php
                                    ...
                                    if (auth()->user()->role == 'CO' || auth()->user()->role == 'CCO')
                                        $display_accounts = $center_accounts;
                                    elseif (auth()->user()->role == 'SO' || auth()->user()->role == 'SCO')
                                        $display_accounts = $froshat_accounts;
                                    elseif (auth()->user()->role == 'MO')
                                        $display_accounts = $mo_accounts;
                                    else
                                        $display_accounts = $sp_accounts;
                                @endphp
                                    ...
                                    <tr class="ur{{ $account->id }}">
                                        ...
                                        <td>
                                            <div class="d-flex align-items-center">
                                                <div class="avatar-sm bg-light-primary text-primary rounded-circle d-flex align-items-center justify-content-center mr-3"
                                                    style="width: 35px; height: 35px; font-weight: 800;">
                                        ...
                                        <td><span class="small text-muted"><i class="feather icon-map-pin mr-1"></i>
                                                {{ $account->address }}</span></td>
                                        <td>
                                                ...
                                                @if($t->remaining < 0)
                                                    <div class="mb-1">
                                                        ...
                                                        <span class="font-weight-bold text-danger"
                                                            dir="ltr">{{ number_format(abs($t->remaining), 2) }}</span>
                                                    </div>
                                                @endif
                                            @endforeach
                                        </td>
                                                ...
                                                @if($t->remaining > 0)
                                                    <div class="mb-1">
                                                        ...
                                                        <span class="font-weight-bold text-success"
                                                            dir="ltr">{{ number_format($t->remaining, 2) }}</span>
                                                    </div>
                                                @endif
                                            @endforeach
                                        </td>
    ...
    <div class="modal fade" id="accountModal" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content QBIC-modal-content">
                ...
                <form
                    action="{{ $accountEdit ? '/dashboard/different-account/' . $accountEdit->id : '/dashboard/different-account' }}"
                    method="post">
                    @csrf
                    @if($accountEdit) @method('PATCH') @endif
                    <div class="modal-body">
                        <div class="form-group mb-4">
                            <label class="font-weight-bold text-muted small uppercase mb-2 d-block">نام کامل شخص یا
                                شرکت</label>
                            <div class="input-group">
                                <div class="input-group-prepend">
                                ...
                                <input type="text" name="name" value="{{ $accountEdit ? $accountEdit->name : old('name') }}"
                                    class="form-control-modern border-left-0 w-100" placeholder="مثلا: شرکت ساختمانی خورشید"
                                    required>
                            </div>
                        </div>

                        <div class="form-group mb-4">
                            <label class="font-weight-bold text-muted small uppercase mb-2 d-block">شماره تماس
                                (فعال)</label>
                            <div class="input-group">
                                <div class="input-group-prepend">
                                    <span class="input-group-text bg-light border-right-0"><i
                                ...
                                <input type="text" name="phone"
                                    value="{{ $accountEdit ? $accountEdit->phone : old('phone') }}"
                                    class="form-control-modern border-left-0 w-100" placeholder="07XXXXXXXX" required
                                    dir="ltr">
                            </div>
                        </div>
                        ...
                        <div class="form-group mb-4">
                            ...
                            <textarea name="address" class="form-control-modern w-100" rows="3"
                                placeholder="ولایت، شهر، جاده..."
                                required>{{ $accountEdit ? $accountEdit->address : old('address') }}</textarea>
                        </div>

                        <div class="form-group">
                            <label class="font-weight-bold text-muted small uppercase mb-2 d-block">یادداشت (Note)</label>
                            <textarea name="note" class="form-control-modern w-100" rows="3"
                                placeholder="یادداشت یا توضیحات اضافی را بنویسید...">{{ $accountEdit ? $accountEdit->note : old('note') }}</textarea>
                        </div>
                    </div>
    ...
    <div class="modal fade" id="differentAccountNoteModal" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content QBIC-modal-content">
                ...
                <div class="modal-body p-4">
                    <form id="differentAccountNoteForm" method="POST" action="">
                        @csrf
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $(document).on('click', '.btn-different-account-note', function () {
                ...
                $('#noteModalAccountName').html('<i class="feather icon-file-text mr-1"></i> یادداشت: ' + name);
                $('#noteAccountId').val(id);
                $('#noteTextarea').val(note || '');
                $('#differentAccountNoteForm').attr('action', '/dashboard/different-account/note/' + id);
            });
