    ...
    <div class="container-fluid">
        ...
        <div class="account-hero">
            <div class="d-flex align-items-center">
                <div class="rounded-circle bg-white text-primary d-flex align-items-center justify-content-center mr-4"
                    style="width: 70px; height: 70px; font-size: 30px; font-weight: 900;">
                    {{ mb_substr($account->name, 0, 1) }}
                </div>
                <div>
                    <h2 class="mb-1 font-weight-bold text-white">{{ $account->name }}</h2>
                    <div class="small opacity-80"><i class="feather icon-phone mr-1"></i> {{ $account->phone }} | <i
                            class="feather icon-map-pin mr-1"></i> {{ $account->address }}</div>
                </div>
            </div>
        ...
        <div class="d-flex justify-content-between align-items-center mb-4 hideOnPrint">
            ...
            <div class="btn-group">
                ...
                <a href="/dashboard/different-account-payments-all/{{$account->id}}"
                    class="btn btn-white shadow-sm border ml-1">نمایش همه</a>
            </div>
        </div>
                        ...
                        <tbody>
                            @foreach($payments as $pa)
                                <tr>
                                    ...
                                    <td>
                                        <div class="small text-muted" dir="ltr">1 {{ $pa->currency_code }} =
                                            {{ number_format($pa->exchange_rate, 4) }} USD</div>
                                    </td>
                                    <td>
                                    ...
                                    <td class="text-center hideOnPrint">
                                        <div class="btn-group">
                                            ...
                                            @if($pa->ledger_transaction_id)
                                                <a href="{{ route('accounting.journals.show', $pa->ledger_transaction_id) }}"
                                                    target="_blank" class="btn btn-sm btn-light-success text-success"
    ...
    <div class="modal fade" id="transactionModal" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content QBIC-modal-content">
                ...
                <form
                    action="{{ $paymentEdit ? '/dashboard/different-account-payments/' . $paymentEdit->id : '/dashboard/different-account-payments' }}"
                    method="post">
                    @csrf
                    ...
                    <div class="modal-body">
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="number" step="any" name="amount"
                                        value="{{ $paymentEdit ? $paymentEdit->amount : '' }}"
                                        class="form-control-modern w-100" required>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label class="small font-weight-bold text-muted">ارز (Currency)</label>
                                    <select name="currency_code" id="currency_code" class="form-control-modern w-100"
                                        required>
                                        @foreach($currencies as $curr)
                                            <option value="{{ $curr->code }}" data-rate="{{ $curr->exchange_rate }}" {{ ($paymentEdit && $paymentEdit->currency_code == $curr->code) ? 'selected' : '' }}>
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="number" step="0.000001" name="exchange_rate" id="exchange_rate"
                                        value="{{ $paymentEdit ? $paymentEdit->exchange_rate : '1.000000' }}"
                                        class="form-control-modern w-100" required {{ ($paymentEdit && $paymentEdit->currency_code == 'USD') ? 'readonly' : '' }}>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="date" name="date"
                                        value="{{ $paymentEdit ? $paymentEdit->date : date('Y-m-d') }}"
                                        class="form-control-modern w-100" required>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="text" id="calculated_usd"
                                        class="form-control-modern w-100 bg-light font-weight-bold text-primary" readonly
                                        value="0.00">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-group mt-3">
                            ...
                            <textarea name="description" class="form-control-modern w-100" rows="2"
                                placeholder="مثلا: بابت کرایه موتر یا خرید وسایل..."
                                required>{{ $paymentEdit ? $paymentEdit->description : '' }}</textarea>
                        </div>
                    </div>
