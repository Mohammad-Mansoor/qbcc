...
<body>
    <table>
        <tr>
            <td colspan="1" align="center" valign="middle"
                style="border: none; background-color: #ffffff; height: 70pt;">
        ...
        <tr>
            ...
            <td colspan="2" class="meta-value" style="height: 24pt;">
                {{ \Carbon\Carbon::parse($bill->date)->format('Y-m-d') }}</td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td colspan="2" class="meta-value" style="height: 24pt;">
                {{ $bill->status == 'open' ? 'باز (Open)' : 'بسته (Closed)' }}</td>
        </tr>
        ...
        <tbody>
            @forelse($purchases as $index => $purchase)
                ...
                <tr style="background-color: {{ $rowBgColor }};">
                    ...
                    <td class="text-center" style="height: 26pt; background-color: {{ $rowBgColor }};">
                        {{ $purchase->purchase_date }}</td>
                    <td class="text-center font-bold" style="height: 26pt; background-color: {{ $rowBgColor }};">
                        {{ $purchase->materialType->material_type ?? 'N/A' }}</td>
                    <td class="text-center" style="height: 26pt; background-color: {{ $rowBgColor }};">
                        {{ $purchase->materialCategory->material_category ?? '—' }}</td>
                    <td class="text-center" style="height: 26pt; background-color: {{ $rowBgColor }};">
                        {{ $purchase->warehouse->name ?? 'N/A' }}</td>
            ...
            <tr>
                ...
                <td class="text-left font-bold" style="background-color: #f1f5f9; direction: ltr; height: 26pt;">
                    {{ number_format($purchases->sum('quantity'), 2) }}</td>
                <td class="meta-label" style="height: 26pt;">مبلغ کل (USD):</td>
                <td class="text-left font-bold"
                    style="background-color: #eff6ff; color: #2563eb; direction: ltr; height: 26pt;">
    ...
    <table>
        <tr>
            <td colspan="6" class="report-title" style="height: 25pt; font-size: 13pt;" valign="middle">تاریخچه تادیات و
                پرداخت‌های بل خرید (Payment History)</td>
        </tr>
        <thead>
            <tr>
        ...
        <tbody>
            ...
            @if($sp)
                <tr>
                    ...
                    <td class="text-center" style="height: 22pt;">
                        {{ $sp->type == 'رسید' ? 'رسید (Inflow)' : 'گرفت (Outflow)' }}</td>
                    <td class="text-left" style="height: 22pt; padding-left: 10px; direction: ltr;">
                        {{ number_format($pay->allocated_amount, 2) }} {{ $sp->currency_code }}</td>
                    <td class="text-center" style="height: 22pt; direction: ltr;">
                        {{ number_format($pay->exchange_rate, 8) }}</td>
                    <td class="text-left font-bold"
                        style="height: 22pt; padding-left: 10px; direction: ltr; color: #16a34a;">
            ...
            <tr>
                ...
                <td class="text-left font-bold"
                    style="background-color: #ecfdf5; color: #16a34a; direction: ltr; height: 24pt;">
            ...
            <tr>
                ...
                <td class="text-left font-bold"
                    style="background-color: #fef2f2; color: #dc2626; direction: ltr; height: 24pt;">
