            ...
            <td style="border: none; padding: 0;">
                ...
                <div class="content-wrapper">
                    ...
                    <table class="meta-table">
                        <tr>
                            <td style="width: 33%; text-align: center; border-left: 1px solid #e2e8f0;">
                                ...
                                <p class="meta-val-primary" style="font-family: monospace; color: #2563eb; direction: ltr;">{{ $bill->bill_number }}</p>
                            </td>
                            <td style="width: 33%; text-align: center; border-left: 1px solid #e2e8f0;">
                                <span class="meta-label">وضعیت بل</span>
                                <p class="meta-val-primary" style="color: {{ $bill->status == 'open' ? '#16a34a' : '#dc2626' }};">
                                    {{ $bill->status == 'open' ? 'باز (Open)' : 'بسته (Closed)' }}
                                </p>
                            </td>
                            <td style="width: 33%; text-align: center;">
                                <span class="meta-label">تاریخ صدور (Date)</span>
                                <p class="meta-val-primary" style="direction: ltr;">{{ \Carbon\Carbon::parse($bill->date)->format('d M Y') }}</p>
                            </td>
                        </tr>
                    </table>
                        ...
                        <tbody>
                            @forelse($purchases as $index => $purchase)
                                <tr>
                                    ...
                                    <td class="text-center">{{ $purchase->materialCategory->material_category ?? '—' }}</td>
                                    <td class="text-center">{{ $purchase->warehouse->name ?? 'N/A' }}</td>
                                    <td class="text-center font-bold" style="direction: ltr;">{{ number_format($purchase->quantity, 2) }} kg</td>
                                    <td class="text-center" style="direction: ltr;">${{ number_format($purchase->price_per_kilo, 2) }}</td>
                                    <td class="text-center font-bold text-dark" style="direction: ltr;">${{ number_format($purchase->total, 2) }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="8" class="text-center" style="color: #64748b;">هیچ خرید مواد خامی به این بل متصل نشده است.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                        ...
                        <tr>
                            ...
                            <td style="width: 50%;">
                                <table class="totals-table text-right">
                                    <tr>
                                        <td class="text-left font-bold" style="direction: ltr;">{{ number_format($purchases->sum('quantity'), 2) }} kg</td>
                                        <td class="font-bold" style="color: #64748b;">مجموع کل وزن:</td>
                                    </tr>
                                    <tr class="grand-total">
                                        <td class="text-left font-bold" style="direction: ltr;">${{ number_format($bill->total_amount, 2) }}</td>
                                        <td class="font-bold">مبلغ کل قابل تادیه (USD):</td>
                                    </tr>
                                    <tr>
                                        <td class="text-left font-bold" style="direction: ltr; color: #16a34a;">${{ number_format($bill->paid_amount, 2) }}</td>
                                        <td class="font-bold" style="color: #64748b;">مجموع پرداخت شده:</td>
                                    </tr>
                                    <tr>
                                        <td class="text-left font-bold" style="direction: ltr; color: #dc2626;">${{ number_format($bill->remaining_balance, 2) }}</td>
                    ...
                    <div style="margin-top: 30px; border-top: 2px solid #e2e8f0; padding-top: 20px;">
                        <h4 style="font-size: 12pt; color: #0f172a; margin-bottom: 10px;">تاریخچه تادیات و پرداخت‌های بل خرید (Payment History)</h4>
                        <table class="ledger-table">
                            <thead>
                                <tr>
                                    <th>تاریخ پرداخت</th>
                            ...
                            <tbody>
                                @foreach($bill->allocations as $pay)
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center" style="direction: ltr;">{{ number_format($pay->allocated_amount, 2) }} {{ $sp->currency_code }}</td>
                                        <td class="text-center" style="direction: ltr;">{{ number_format($pay->exchange_rate, 8) }}</td>
                                        <td class="text-center font-bold" style="direction: ltr; color: #166534;">${{ number_format($pay->base_allocated_amount, 2) }}</td>
                                    </tr>
                                    @endif
                                @endforeach
                            </tbody>
