...
class AddAdvanceFieldsToKachaeePaymentsTable extends Migration
{
    public function up()
    {
        Schema::table('kachaee_payments', function (Blueprint $table) {
        ...
        DB::table('kachaee_payments')->chunkById(100, function ($payments) {
            foreach ($payments as $payment) {
                $orig = $payment->original_amount ?: ($payment->amount ?: $payment->amount_af);
                DB::table('kachaee_payments')
                    ->where('id', $payment->id)
